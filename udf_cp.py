...
def transtime(time):
  #把time按空格分开，共3个字段
   ...
   if len(time) == 4:
     ...
     tm_d = time[0].strip('\xe6\x9c\x88').split('-')
     tmonth  = tm_d[1]
     if len(tmonth) == 1:
       tmonth = '0' + tmonth
     pass
     ...
     #专门处理上午12和下午12
     print '下午'
     if time[3] == '\xe4\xb8\x8b\xe5\x8d\x88':
         thour = int(thour) + 12
     thour = str(thour)
     pass
   pass
   ...
   if len(time) == 3:
     ...
     if time[0].split('-')[1] == 'AUG' or time[0].split('-')[1] == '8\xe6\x9c\x88':
       tmonth = '08'
     pass
     if time[0].split('-')[1] == 'OCT' or time[0].split('-')[1] == '10\xe6\x9c\x88':
       tmonth = '10'
     pass
     tday = time[0].split('-')[0]
     if len(tday) == 1:
